...
<body>
	<script>alert('Atenção ao DELETAR! Um jogo será apagado juntamente com os registros que tiverem uma restrição de integridade ON DELETE. Nesse caso especifico há uma nenhuma restrição em outras tabelas!')</script>
	<h1><center>Menu: Deletar Jogo</center></h1>
			...
			if($result->num_rows > 0) {
				while($row = $result->fetch_assoc()) {
					echo "<tr>
						...
						<td>
							<a href='remove_jogo.php?numero_jogo=".$row['numero_jogo']."'><button type='button'>Remover Dados</button></a>
						</td>
					</tr>";
				}
			} else {
